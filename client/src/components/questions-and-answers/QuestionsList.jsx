...
class QuestionsList extends React.Component {
  ...
  showMoreQuestions() {
    utils
      .fetchQuestions(this.props.productId, this.state.questionPage + 1, 2)
      .then((questions) => {
  ...
  renderQuestions() {
    return this.state.questions
      .filter((question) =>
        question.question_body.toLowerCase().includes(this.props.searchFilter.toLowerCase())
      )
  ...
  render() {
    return (
      <div style={{ overflow: 'auto', maxHeight: '80vh' }}>
        {!this.state.questions ? <p>LOADING...</p> : this.renderQuestions()}
        {!this.state.questions ? null : this.state.areMoreQuestions ? (
        ...
        <QuestionModal
          ...
          productId={this.props.productId}
        />
        <SuccessModal onClose={() => this.openSuccessModal(false)} show={this.state.showSuccess} />
        <TextButton onClick={() => this.openQuestionModal(true)}>ADD A QUESTION ＋</TextButton>
      </div>
    );
  }
}
